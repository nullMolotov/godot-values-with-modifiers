...
var float_with_modifiers: FloatWithModifiers:
	set(new):
		...
		var modifiers := float_with_modifiers._modifiers
...
func _on_modifier_list_item_selected(index: int) -> void:
	...
	remove_button.disabled = not float_with_modifiers.has_modifier(selected_modifier)
...
func _on_set_button_pressed() -> void:
	if selected_modifier:
		float_with_modifiers.modifier_edit(
			selected_modifier,
			mod_value.value,
		...
		float_with_modifiers.rename_modifier(selected_modifier, mod_name.text)
	else:
		if float_with_modifiers.has_modifier(mod_name.text):
			float_with_modifiers.modifier_edit(
				mod_name.text,
				mod_value.value,
				mod_operation.selected,
			...
			float_with_modifiers.add_modifier(
				mod_name.text,
				mod_value.value,
				mod_operation.selected,
				mod_priority.value
			)
			float_with_modifiers.set_modifier_enabled(mod_name.text, mod_enabled.button_pressed)

func _on_line_edit_text_submitted(new_text: String) -> void:
	if float_with_modifiers.has_modifier(new_text):
		remove_button.disabled = false
	else:
		remove_button.disabled = true
...
func _on_remove_button_pressed() -> void:
	if selected_modifier:
		float_with_modifiers.remove_modifier(selected_modifier)
